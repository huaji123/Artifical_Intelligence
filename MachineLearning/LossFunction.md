...
#### 3.2.2 交叉熵损失 Cross Entropy Loss（多分类）
...
#### 3.2.5 Hinge Loss（二分类）

##### 公式：L = max(0, 1-ys)，基于输出标签的表示方式为y={-1，+1}

ys>0时，s大于等于1或者小于等于-1时，都是分类器确定的分类结果，此时的损失函数loss为0； ys<0时，预测错误，loss大

#### 3.2.6 Exponential Loss（二分类）

##### 公式：L = exp(-ys)

特点：Exponential Loss是指数下降的，因此梯度较其他Loss更大一些；容易放大异常点对模型的影响
